<template>
  <div class="audit-container">
    ...
    >
      <div :key="selectFlowNode.id" class="card-body">
        <!--审批人-->
          ...
          <template v-else>
            <div v-for="(item, index) of selectFlowNode.children" :key="'sfn' + index" class="body-item">
              ...
              <template v-if="!_.isNil(item.auditResult)">
                <el-form disabled label-position="left" label-width="82px">
...
export default {
  ...
  methods: {
    //需要审核的情况下,获取审核表单
    async getFormStyle() {
      let res = await this.axios.workflow.get(`/oa/audit/style/template/${this.id}`)
      let datas = res.data
      if (!datas) {
        return
      }
    ...
    async getDetail() {
      let res = await this.axios.workflow.get(`/oa/form/process/get/${this.id}`)
    ...
    async auditSuccess(formData) {
      ...
      await this.axios.workflow.post(`/task/complete/${this.id}`, formData)
