...
const bimTools = require('bim-tools')
import 'bim-tools/dist/bimTools.css'
...
export default {
  ...
  methods: {
    async refresh() {
      ...
      let res = await axios.get(`/bim-storage/model/getCacheResult/${this.modelId}`)
      let path = res?.data?.data?.modelPath || null
      let cacheFileFlag = res?.data?.data?.cacheFileFlag || null
      let cacheTreeFlag = res?.data?.data?.cacheTreeFlag || null
      this.loadView(path)
      if (!cacheFileFlag) {
    ...
    async loadView(modelLocalUrl) {
      ...
      let cameraInfo = JSON.parse(localStorage.getItem('bimModelCamera'))
    ...
    async saveModelAllElements() {
      ...
      let res = await axios.get(`/bim-storage/model/getElementList/${this.modelId}`).catch(e => {
        console.log(e)
      })
      let allEleAry = res.data.data
      let allEleObj = {
        modelId: this.modelId,
        allEleAry
      }
    ...
    async saveLocalUrl(modelId) {
      let localUrl = await Modelo.Model.get(modelId).then(res => res.convertedUrls)
      await axios
    ...
    async saveTreeData(modelId) {
      let treeData = await Modelo.BIM.getTreeInfo(modelId).then(tree => tree)
    ...
    clearElementsColor() {
      if (!window.bimfishModeloApp) return
      let ids = _.map(window.allEleAry, 'id')
