...
<template>
  <div>
    ...
    >
      ...
      <el-button v-if="!isButton" slot="trigger" size="small" type="primary">{{ btnText }}</el-button>
      <div v-if="!isButton" slot="tip" class="el-upload__tip">支持扩展名：{{ acceptText }}</div>
    </el-upload>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async beforeUpload(file) {
      if (this.accept) {
        console.log(!_.some(this.accept.split(','), string => file.name.toLowerCase().includes(string.toLowerCase())))
