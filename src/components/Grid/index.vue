<template>
  <div class="container">
    <component
      :is="columns.length ? 'vxe-grid' : 'vxe-table'"
      ref="xTable"
      v-loading="loading"
      :size="size"
      :row-key="rowKey"
  ...
  props: {
    ...
    checkboxConfig: {
      ...
      default() {
        ...
        return { checkRowKeys: this.checkRowKeys, highlight: true, reserve: true }
      }
    },
    rowKey: {
      type: Boolean,
  ...
  methods: {
    ...
    refresh(changedJumpToFirst = true) {
      ...
      return this.request(body)
        .then(res => {
          if (this.pagination) {
            const { total, records, list } = res.data
            this.total = total
    ...
    checkChange({ records }) {
      ...
      this.$emit('checkboxChange', [...records, ...check])
    }
  }
}
</script>
