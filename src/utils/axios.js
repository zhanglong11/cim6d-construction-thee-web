...
const processApi = create('/cim6d-process' + urlSuffix) // OA相关API
...
export const projectFileUploadUrl = `/cim6d-file${urlSuffix}/file/commonFile/upload`
export const projectFile = create(fileUrl)

// const labour = create(`/cim6d-construction${urlSuffix}-nht/labour`) //劳务模块--测试-连接牛湖涛电脑
export const iotUrl = `/cim6d-iot${urlSuffix}`
...
const iots = create(`/cim6d-iot${urlSuffix}`) // 物联网
const iotEquipmentMonitor = create(iotUrl) // 设备监控
...
function addInterceptor(service) {
  ...
  service.interceptors.request.use(
    config => {
      ...
      if (localStorage.getItem('projectId') && !config.headers['project-id']) {
        config.headers['project-id'] = localStorage.getItem('projectId')
      }
      config.headers['platform'] = localStorage.getItem('projectType')
