    ...
    getPatrolRouteDetail(id) {
      Api.getPatrolRouteDetail(id).then(res => {
        ...
        if (res.data) {
          ...
          this.form.name = this.detailData.routeName
          let patrolRouteDrawingsList = this.detailData.patrolRouteDrawingsList
          let newForms = []

          // 回显图纸信息
          for (let i = 0; i < patrolRouteDrawingsList.length; i++) {
            let item = patrolRouteDrawingsList[i]
            let pointList = item.pointList
            let coordinate = [] // 点位坐标恢复
            for (let j = 0; j < pointList.length; j++) {
              let coordinateArray = pointList[j].patrolPointLocation.split(',')
              coordinate.push([parseFloat(coordinateArray[0]), parseFloat(coordinateArray[1])]) // 字符串坐标需要转换成数字类型
            }
            newForms.push({
              drawingsId: item.drawingsId,
              drawingsName: item.drawingsName,
              imgUrl: item.modelUrl,
          ...
          this.$nextTick(() => {
            for (let i = 0; i < this.forms.length; i++) {
              ...
              this.forms[i].canvasContext = canvas.getContext('2d')
              this.forms[i].canvasContext.lineWidth = 2
              this.forms[i].canvasContext.lineColor = 'red'

              drawPatrolPath(this.forms[i].coordinate, this.forms[i]) // 绘制路线
            }
          })
        }
      })
    ...
    handleSave() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          Api.updatePatrolRoute(params).then(
            res => {
