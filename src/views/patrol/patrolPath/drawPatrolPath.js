...
function drawPath(coordinate, formItem) {
  ...
  context.clearRect(0, 0, 800, 600) // 清空画布内容

  if (coordinate.length > 1) {
    drawLine(context, coordinate) // 绘制圆点之间的连线
  }
  drawCircle(context, coordinate) // 绘制实心圆点
...
// 绘制实心圆点
function drawCircle(context, coordinate) {
  for (let i = 0; i < coordinate.length; i++) {
    context.fillStyle = 'red'
    context.beginPath()
    context.arc(coordinate[i][0], coordinate[i][1], 8, Math.PI * 2, 0, true)
    context.closePath()
    context.fill()
  }
}
