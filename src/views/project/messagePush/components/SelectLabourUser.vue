<template>
  <el-dialog
    top="10vh"
    :title="`添加劳务人员（${selectUserIds.length}/${max}）`"
    width="50%"
    append-to-body
    :visible.sync="modalStatus"
  >
    <el-form ref="formRef">
      <el-form-item v-if="selectUserIds.length > 0" label="已选劳务人员">
        <div class="tags">
...
export default {
  ...
  computed: {
    list() {
      ...
      treeForEach(data, e => {
        ...
        e.isUser = e.organType === 6
        e.disabled = e.organType !== 6 // 禁止部门选中事件
        if (e.isUser) {
          delete e.children
        }
      })
  ...
  mounted() {
    this.queryLabourTreeListByWx()
  },
  //方法集合
  methods: {
    // 民工微信用: 公司-班组-人员返回树结构
    async queryLabourTreeListByWx() {
      let result = await projectApi.queryLabourTreeListByWx({})
