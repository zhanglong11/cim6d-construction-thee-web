<template>
  <div class="container">
        ...
        <el-row :gutter="20" inline>
          ...
          <el-col :span="8">
            <el-form-item label="项目类型" prop="type" required>
              <el-select v-model="formData.type" placeholder="请选择" class="w100" @change="changeType">
          ...
          <el-col :span="8">
            <el-form-item label="参与类型" prop="participantsNature" required>
              <el-select v-model="formData.participantsNature" filterable placeholder="请选择" class="w100">
                <template v-for="item in participantsNatureList">
                  <el-option v-if="!item.disable" :key="item.value" :label="item.label" :value="item.value" />
                </template>
              </el-select>
            </el-form-item>
          ...
          <el-col :span="12">
            <el-form-item label="项目负责人" prop="principal" required>
              <el-select v-model="formData.principal" placeholder="请选择" class="w100" @change="changePrincipal">
    ...
    <el-card class="card-item">
      ...
      <el-card class="card-item">
        <div slot="header" class="clearfix">
          ...
          <div class="card-extend">
            <el-button type="primary" icon="el-icon-plus" size="mini" @click="onUploadImg('3')"> 上传图片 </el-button>
            <el-button
              type="text"
      ...
      <el-card class="card-item">
        <div slot="header" class="clearfix">
          ...
          <div class="card-extend">
            <el-button type="primary" icon="el-icon-plus" size="mini" @click="onUploadImg('4')"> 上传图片 </el-button>
            <el-button
              type="text"
              icon="el-icon-picture"
              class="type-icon"
    ...
    <div v-if="projectId" class="footer-btn">
      <el-button @click="goBack">返回</el-button>
      <el-button :loading="btnLoading" type="primary" @click="saveProject">提交</el-button>
    </div>

    <!-- 上传图片 模态框 -->
    <el-dialog
      append-to-body
      :title="uploadImgTitle"
      :visible.sync="onUploadImgVisible"
    ...
    <el-dialog
      append-to-body
      title="地图选点"
      :visible.sync="mapData.dialogVisible"
      width="1000px"
    ...
    >
      <Map
        ...
        @choosedLocation="handleChosenLocation"
      />
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      pca: pca, // 省市区初始化数据
      projectId: this.isHome ? '' : this.$route.params.id || localStorage.getItem('currentProjectSettingsId'), // 项目id
      ...
      cacheTimer: '', // 自动缓存定时器
      participantsNatureList: [], // 参与类型下来列表
      formData: {
        address: '', // 省市区
        participantsNature: '', // 参与类型
  ...
  computed: {
    ...
    serviceList() {
      let serviceList = this.$store.state.userPower.serviceList
      return serviceList
    },
    projectType() {
  ...
  methods: {
    // 初始化
    init() {
      let projectTypeList = this.$getArgList('projectType')
      this.projectTypeList = _.filter(projectTypeList, e => this.serviceList.includes(e.value))
    ...
    // 选择项目负责人
    changePrincipal(e) {
      let item = _.find(this.principalList, { value: e })
      if (!item) {
        return
      }
      this.formData.telephone = item.mobile
    },
    // 选择项目类型
    changeType(e) {
      let participantsNatureList = this.$getArgList(`${e}_participantType`) || []
      this.participantsNatureList = participantsNatureList
      if (
        !_.find(participantsNatureList, {
          value: this.formData.participantsNature
        })
      ) {
        this.formData.participantsNature = ''
      }
    },
    // 打开地图
    openMap() {
    ...
    async getProject() {
      ...
      datas.address = datas.address ? datas.address.split('_') : []
    ...
    async saveProjectSuccess() {
      ...
      try {
        ...
        this.$event.emit('projectListChange')
        this.btnLoading = false
        if (this.isHome) {
          this.$store.dispatch('queryProjectList')
          this.$store.commit('setAddProjectDialog', false)
        } else {
          this.goBack()
        }
      } catch (e) {
      ...
      this.$confirm('确认删除选中的图片?', '提示', {
        ...
        .then(() => {
          if (type === 'single') {
            ...
            // 批量删除
            this.imageListTemp[this.activeName] = _.difference(
              this.imageListTemp[this.activeName],
    ...
    getPrincipalList() {
      ...
      projectApi.getPrincipalList(param).then(res => {
        this.principalList = res.data.map(item => {
          return { value: item.userId, label: item.userRealName, mobile: item.mobile }
        })
      })
    },
    // 返回
