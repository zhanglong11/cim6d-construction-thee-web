          ...
          <ul>
            <li v-for="item in items.children" :key="item.id">
              <div class="box">
                ...
                <p class="action">
                  <el-tooltip class="item" effect="dark" content="编辑表单" placement="bottom-start">
                    <el-button type="text" icon="el-icon-edit-outline" @click="editForm(item.id)"></el-button>
                  </el-tooltip>
                  <el-tooltip class="item" effect="dark" content="删除表单" placement="bottom-start">
...
export default {
  ...
  methods: {
    ...
    addForm() {
      ...
      this.$router.push({
        name: 'SystemFormListAdd'
      })
    },
    // 修改表单
    editForm(id) {
      this.$router.push({
        name: 'SystemFormListEdit',
        query: {
          id
        }
      })
