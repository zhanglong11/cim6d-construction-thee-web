...
export default {
  ...
  methods: {
    ...
    async getSourcePower() {
      ...
      let { data } = await roleApi.getSystemSourcePower(params)
      data = _.find(data, { serviceId: 'base' }, {}).children || []
      treeForEach(data, item => {
        if (!item.powerId) item.powerId = item.id
      })

      this.sourcePower = data
    },
    // 角色分类&角色树
    async getTreePermissionPower() {
      ...
      const result = await roleApi.getTreePermissionPower({})
    ...
    async save() {
      ...
      const arr = this.selectPower.map(item => {
        return {
          ..._.pick(item, ['serviceId', 'powerKey']),
          powerId: item.powerId,
          roleId: this.roleId
        }
      })
      if (arr.length === 0) {
        await roleApi.resetRoleBind(this.roleId, 'base')
      } else {
