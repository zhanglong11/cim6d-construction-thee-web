  ...
  <div>
    ...
    >
      <template #default="{ node, data: nodeData }">
        <span :id="data.powerId">
          <span class="el-icon-star-on" style="color: #00800d" />
  ...
  methods: {
    handleCheck(node, { checkedKeys, halfCheckedKeys, checkedNodes, halfCheckedNodes }) {
      ...
      this.$emit('change', {
        ...
        removedPowerKeys,
        currentCheckedKeys,
        currentCheckedNodes: [...checkedNodes, ...halfCheckedNodes].filter(item => item.powerId !== 'root')
      })
      this.formatTree()
    },
    calculateCheckedNodes() {
      ...
      treeForEach(this.computedData, item => {
        if (powerKeyHash[item.powerId] && _.size(item.children)) {
          delete powerKeyHash[item.powerId]
        }
      })
