        ...
        <vxe-table-column title="操作" width="150" fixed="right">
          <template #default="{ row }">
            <template v-if="row.isBind">
              <el-button v-if="$hasPower('safeBIMUnlink')" type="text" @click="unLink(row.id)">取消关联</el-button>
              <el-button type="text" @click="watchLink(row.id)">查看</el-button>
            </template>
            <template v-else>
              <el-button v-if="$hasPower('safeBIMLink') && row.status !== 0" type="text" @click="link(row.id)"
                >关联</el-button
              >
            </template>
          </template>
...
export default {
  ...
  methods: {
    getAllElement() {
      this.allEleAry = this.$refs.bim.getModelAllElementAry()
      this.isReady = true
    },
    //刷新数据
    refresh() {
    ...
    bimTreeChange(v) {
      ...
      this.$refs.bim.showGuid(ele.map(c => c.id))
    },
    //关联
    async link(id) {
