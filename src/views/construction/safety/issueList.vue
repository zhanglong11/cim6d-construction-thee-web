  ...
  <div class="container">
    ...
    <div class="tooltips">
      <el-form inline>
        ...
        <el-form-item>
          <el-button v-if="$hasPower('SafetyIssueAdd')" type="primary" icon="el-icon-plus" @click="addItem()"
            >新建</el-button
          >
        </el-form-item>
      </el-form>
      ...
      <vxe-table-column title="操作" width="200" fixed="right">
        <template #default="{ row }">
          <template v-if="row.status === 0 && userId === row.creator">
            <el-button v-if="$hasPower('SafetyIssueRelease')" type="text" @click="handleRelease(row)">下达</el-button>
            <el-button v-if="$hasPower('SafetyIssueEdit')" type="text" @click="addItem(row)">编辑</el-button>
            <el-button v-if="$hasPower('SafetyIssueDelete')" type="text" @click="toDelete(row)">删除</el-button>
          </template>
          <template v-if="$hasPower('SafetyIssueRectify') && row.status === 1 && userId === row.rectifyPrincipal">
            <el-button type="text" @click="toDetail(row, 'rectify')">整改</el-button>
          </template>
          <template v-if="$hasPower('SafetyIssueReview') && row.status === 2">
            <el-button type="text" @click="toDetail(row, 'review')">复查</el-button>
          </template>
          <el-button v-if="$hasPower('SafetyIssueDetail')" type="text" @click="toDetail(row, '')">查看</el-button>
        </template>
      </vxe-table-column>
    </Grid>
    <AddIssue
...
import AddIssue from '@/views/construction/quality/components/addIssue' //添加/编辑弹窗
import SafetyIssueDetail from './components/safetyIssueDetail' //详情
export default {
  // import引入的组件需要注入到对象中才能使用
  name: 'SafetyIssueList',
  components: { AddIssue, SafetyIssueDetail },
  data() {
    // 这里存放数据
    return {
  ...
  methods: {
    ...
    async addItem(row) {
      ...
      if (id) {
        ...
        this.rowTableData = [{ ...row, projectId: this.projectId, inspectType: 2, fileId: res.data.fileId }]
      } else {
        this.rowTableData = []
      }
