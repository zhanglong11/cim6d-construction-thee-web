...
import { getMonyOneAndEndDate } from '@/utils'
export default {
  name: 'CheckEdit',
  components: {
    // SelectLabourWorker
  ...
  data() {
    return {
      ...
      reportDate: [
        moment(getMonyOneAndEndDate(new Date()).monthStartDate).format('YYYY-MM-DD'),
        moment(getMonyOneAndEndDate(new Date()).monthEndDate).format('YYYY-MM-DD')
      ],
      status: '',
