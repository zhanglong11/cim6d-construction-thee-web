      ...
      <el-row :gutter="15">
        <el-col :span="inDialog ? 22 : 10">
          <div class="item-replay">
            <h3 class="title-h3">安全月度联合检查回复表</h3>
            <div class="from">
              <table border="0">
                <tr>
                  <td width="20%">工程名称</td>
                  <td width="40%">
                    <span class="td-span">{{ form.engineeringName }}</span>
                  </td>
                  <td width="20%">检查区域</td>
                  <td width="20%">
              ...
              <el-row>
                <el-col>
                  <el-table
                    ...
                    style="width: 100%"
                    border
                    :header-cell-style="{ borderColor: '#fff' }"
                  >
                    ...
                    <el-table-column label="隐患整改" align="center">
                      ...
                      <el-table-column prop="rectificationMeasures" label="整改措施" width="120"> </el-table-column>
...
export default {
  ...
  created() {
    if (this.id) {
      Api.getcheckDetail(this.id).then(res => {
        this.form = res.data
        this.wrapperInfo()
      })
    }
  ...
  methods: {
    ...
    async goDwon() {
      let res = await Api.getSafetyMonthDwon(this.id)
      let resFForm = await Api.getSafetyMonthDwonFrom(this.id)
      saveAs(res.data, this.fileName ? this.fileName : 'download.doc')
      saveAs(resFForm.data, this.fileNameForm ? this.fileNameForm : 'download.doc')
    },
    //验收申请详情页
    wrapperInfo() {
      let info = this.form
