<template>
  <div id="main">
    ...
    <div v-if="edit" class="btnBox">
      <!-- <el-button v-if="mapDelete" type="primary" round @click="removePolygon"> 删除围栏 </el-button> -->
      <el-button v-if="mapAdd" type="primary" round @click="drawPolygon"> 构建围栏 </el-button>
...
export default {
  ...
  methods: {
    ...
    async drawGridAll(isClear) {
      ...
      await this.$nextTick()
      console.log(this.multiplePoint)
    ...
    drawPolygon() {
      ...
      // let gridName = this.gridName ? this.gridName : '围栏区域'
      ...
      const fillColor = this.color || '#409eff'
        ...
        this.$nextTick(() => {
          ...
          this.webgisOlMap.drawFeature_OL(
            ...
            feature => {
              ...
              this.addMarker(currentDraw, center, area)
        ...
        this.$nextTick(() => {
          ...
          this.webgisOlMap.drawFeature_OL(
            ...
            feature => {
              const extent = feature.getGeometry().getExtent()
              const center = this.webgisOlMap.getCenterFromExtent_OL(extent)
              this.addMarkerCircle(currentDraw, center)
              this.currentDraw = currentDraw
              // console.log('记录本次绘制的区域ID', this.currentDraw)
              this.saveInfoArr.push({
                text: gridId,
    ...
    async showdrawGrid() {
      ...
      let innerHTML = `<div class="pupup-header">
              <span class="title">${data.sosType}</span>
            </div>
            <div class="popup-body">
              <div class="popup-item">
                <label class="popup-label">原因：</label>
                <div class="popup-desc">${data.sosType}</div>
              </div>
               <div class="popup-item">
                <label class="popup-label">设备：</label>
                <div class="popup-desc">${data.deviceId}</div>
              </div>
              <div class="popup-item">
                <label class="popup-label">位置：</label>
                <div class="popup-desc">${data.location}</div>
              </div>
              <div class="popup-item">
                <label class="popup-label">时间：</label>
                <div class="popup-desc">${data.createTime}</div>
              </div>
            </div>
          `
      // this.webgisOlMap.setSelectPropertyOverLayContent_OL(data.id, innerHTML)
    },
