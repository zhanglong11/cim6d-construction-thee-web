    ...
    <el-table v-loading="loading" :data="tableData" border stripe size="small">
      ...
      <el-table-column label="操作" width="260" fixed="right" align="center">
        <template slot-scope="scope">
          <div>
            <el-switch
              v-if="scope.row.creator === currentId"
              :value="scope.row.inPublic === 1"
              style="margin-right: 8px; vertical-align: 1px"
              inactive-text="是否公开"
              @change="changeIsWatch(scope.row)"
            />
...
export default {
  ...
  methods: {
    ...
    async changeIsWatch(row) {
      await Api.updateData({ id: row.id, inPublic: row.inPublic === 1 ? 2 : 1 })
