    ...
    <div class="task-list-wrap" style="margin-top: 10px">
      ...
      <div class="task-list">
        <div class="headContainer">
          <div class="formWrapper">
            <el-form ref="tableForm" :inline="true" :model="tableForm">
              <div>
                <el-form-item label="紧急程度" label-width="100px" prop="riskCategory" style="margin-right: 20px">
                  <el-select v-model="tableForm.riskCategory" placeholder="请选择">
                    <el-option
                ...
                <el-form-item label label-width="0" prop="keyWords" style="margin-right: 20px">
                  <el-input v-model="tableForm.keyWords" placeholder="安全文明检查编号/名称" style="width: 200px">
        ...
        >
          ...
          <vxe-table-column title="安全文明检查编号" field="civilizationCheckNumber"></vxe-table-column>
          <vxe-table-column title="安全文明问题" field="describe"></vxe-table-column>
          <vxe-table-column title="紧急程度" field="urgency">
            <template slot-scope="{ row }">
              <span v-if="row.urgency">{{ urgencyDegree.find(item => item.value === row.urgency).label }}</span>
            </template>
          </vxe-table-column>
          <vxe-table-column title="部位" field="placeName"></vxe-table-column>
          <vxe-table-column title="更新时间" field="updateTime"></vxe-table-column>
...
export default {
  ...
  methods: {
    ...
    async refresh() {
      let res = await Api.getSafetyCivilizationBindPage({
        ...this.tableForm,
        rows: this.pageSize,
    ...
    async handleBind(id) {
      ...
      await Api.safetyCultureBimBind(param).then(() => {
        this.loading = false
    ...
    async watchLink(row) {
      const res = await Api.getCultureGuid({ targetId: row.id })
    ...
    async handleUnbind(id) {
      ...
      let res = await Api.safetyCultureUnbind(param)
