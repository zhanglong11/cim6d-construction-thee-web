  ...
  <div class="main">
    <div class="task-list-wrap">
      ...
      <div class="task-list" style="padding: 0">
        <BIM ref="BIM" idSelect="qualityAcceptance" />
      </div>
    </div>
    <div class="task-list-wrap" style="margin-top: 10px">
      ...
      <div class="task-list">
        <div class="headContainer">
          <div class="formWrapper">
            <el-form ref="tableForm" :inline="true" :model="tableForm">
              <div>
                <el-form-item label="验收类别" label-width="100px" prop="acceptanceType" style="margin-right: 20px">
                  <el-select v-model="tableForm.acceptanceType" placeholder="请选择">
                    <el-option v-for="item in acceptanceType" :key="item.value" :label="item.label" :value="item.value">
                    </el-option>
                  </el-select>
                </el-form-item>
                ...
                <el-form-item label label-width="0" prop="acceptanceName" style="margin-right: 20px">
                  <el-input v-model="tableForm.acceptanceName" placeholder="验收名称关键字" style="width: 200px">
        ...
        >
          ...
          <vxe-table-column title="编号" field="acceptanceApplyNumber"></vxe-table-column>
          <vxe-table-column title="验收名称" field="acceptanceName"></vxe-table-column>
          <vxe-table-column title="验收类别" field="acceptanceType">
            <template slot-scope="{ row }">
              <span>{{
                row.acceptanceType !== null && row.acceptanceType !== undefined
                  ? acceptanceType.filter(item => item.value === row.acceptanceType)[0].label
                  : ''
              }}</span>
            </template>
          </vxe-table-column>
          <vxe-table-column title="验收结果" field="status">
            <template slot-scope="{ row }">
              <span v-if="row.status == 5">合格</span>
              <span v-if="row.status == 6">不合格</span>
            </template>
          </vxe-table-column>
          <vxe-table-column title="更新时间" field="acceptanceDate"></vxe-table-column>
          ...
          <vxe-table-column title="操作">
            <template slot-scope="{ row }">
              <template v-if="row.bindFlag == 1">
                <span v-if="$hasPower('removeQualityAcceptanceRelated')" style="margin: 0 3px"
                  ><a @click="handleUnbind(row.id)">取消关联</a></span
                >
                <span v-if="$hasPower('qualityAcceptanceDetail')" style="margin: 0 3px"
                  ><a @click="watchLink(row)">查看</a></span
                >
              </template>
              <template v-else>
                <span v-if="$hasPower('qualityAcceptanceRelated')" style="margin: 0 3px"
                  ><a @click="handleBind(row.id)">关联</a></span
                >
              </template>
            </template>
          </vxe-table-column>
...
import acceptanceType from '@/lib/acceptanceType'
...
export default {
  ...
  methods: {
    ...
    async refresh() {
      let res = await Api.getQualityAcceptanceBindPage({
        ...this.tableForm,
        rows: this.pageSize,
    ...
    async handleBind(id) {
      ...
      await Api.qualityAcceptanceBimBind(param).then(() => {
        this.loading = false
    ...
    async watchLink(row) {
      const res = await Api.getAcceptGuid({ targetId: row.id })
    ...
    async handleUnbind(id) {
      ...
      let res = await Api.qualityAcceptanceUnbind(param)
