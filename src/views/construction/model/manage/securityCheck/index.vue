  ...
  <div class="main">
    <div class="task-list-wrap">
      ...
      <div class="task-list" style="padding: 0">
        <!-- <div id="securityCheck" ref="securityCheck" class="lo-viewer"></div> -->
        <BIM ref="BIM" idSelect="securityCheck" />
      </div>
    </div>
    <div class="task-list-wrap" style="margin-top: 10px">
      ...
      <div class="task-list">
            ...
            <el-form ref="tableForm" :inline="true" :model="tableForm">
              <div>
                ...
                <el-form-item label label-width="0" prop="keyWords" style="margin-right: 20px">
                  <el-input
                    v-model="tableForm.issueDescription"
                    placeholder="安全检查编号/安全检查问题"
                    style="width: 210px"
                  >
        ...
        >
          ...
          <vxe-table-column title="安全检查编号" field="planNum"></vxe-table-column>
          <vxe-table-column title="安全检查问题" field="issueDescription"></vxe-table-column>
          <vxe-table-column title="紧急程度" field="emergencyLevel">
            <template slot-scope="{ row }">
              <span v-if="row.emergencyLevel">{{
                urgencyDegree.find(item => item.value === row.emergencyLevel).label
              }}</span>
            </template>
          </vxe-table-column>
          ...
          <vxe-table-column title="操作">
            <template slot-scope="{ row }">
              <template v-if="row.bindFlag == '1'">
                <span v-if="$hasPower('removeSafeRelated')" style="margin: 0 3px">
                  <a @click="handleUnbind(row.id)">取消关联</a>
                </span>
                <span v-if="$hasPower('safeRelatedDetail')" style="margin: 0 3px">
                  <a @click="watchLink(row)">查看</a>
                </span>
              </template>
              <template v-else>
                <span v-if="$hasPower('safeCheckRelated')" style="margin: 0 3px">
...
export default {
  ...
  methods: {
    ...
    async refresh() {
      let res = await Api.getSafetyBindPage({
        ...this.tableForm,
        rows: this.pageSize,
    ...
    async handleBind(id) {
      ...
      await Api.safetyCheckBimBind(param).then(() => {
        this.loading = false
    ...
    async watchLink(row) {
      const res = await Api.getCheckGuid({ targetId: row.id })
    ...
    async handleUnbind(id) {
      ...
      await Api.safetyCheckUnbind(param)
