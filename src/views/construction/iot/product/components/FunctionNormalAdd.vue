...
export default {
  ...
  methods: {
    async refresh() {
      let current = await Api.getIotProductAttributeSimpleList(this.product.templateId, 1).then(res => res.data)
      current.forEach(e => (e.type = 1))
      let other = await Api.getIotProductAttributeSimpleList(this.product.templateId, 2).then(res => res.data)
    ...
    async submit() {
      await Api.bindProductStandardAttribute(
        this.product.id,
