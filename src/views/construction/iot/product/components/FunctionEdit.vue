...
export default {
  ...
  created() {
    Api.getIotArgList({ argGroup: 'iot_unit' }).then(res => {
      this.unitList = res.data
    })
    Api.getIotArgList({ argGroup: 'iot_data_type' }).then(res => {
    ...
    async submit() {
      ...
      if (this.form.id) {
        if (this.form.customFlag === '0') {
          await Api.updateIotProductStandard(form)
        } else {
          await Api.updateIotProductCustom(form)
        }
        this.$message.success('修改成功')
      } else {
        await Api.addIotProductCustomAttribute(form)
