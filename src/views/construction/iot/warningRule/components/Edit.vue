      ...
      <li v-for="(attr, index) in list" :key="attr.id">
        ...
        >
          ...
          <vxe-table-column width="120px">
            <template #default="{ row, rowIndex, $table }">
...
export default {
          ...
          return Api.getIotProductAttributeList({
            ...
            await Promise.all(
              res.data.records.map(async attr => {
                ...
                attr.rules = await Api.getIotDeviceWarningRuleList({
  ...
  methods: {
    async saveRow(row, $table) {
      ...
      if (row.id) {
        await Api.updateIotDeviceWarningRule(row).then(res => {})
      } else {
        await Api.addIotDeviceWarningRule(row).then(res => {
          row.id = res.data
        })
      }
    ...
    remove(attr, row, rowIndex) {
      this.$confirm('确认删除该数据?', '提示', {
        ...
        .then(() => {
          Api.deleteIotDeviceWarningRule(row.id).then(res => {
