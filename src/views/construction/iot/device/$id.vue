<template>
  <div>
    ...
    <el-tabs style="margin-top: 10px">
      ...
      <el-tab-pane lazy label="运行状态"><RunStatus :device="device"></RunStatus></el-tab-pane>
    </el-tabs>
  </div>
</template>
...
export default {
  ...
  methods: {
    refresh() {
      Api.getIotDeviceDetail(this.id).then(res => {
    ...
    editAlias() {
      this.$prompt('备注名称', '编辑备注名称', {
        ...
        await Api.updateIotDevice({ ids: [this.id], remark: value })
        this.$message.success('编辑成功')
        this.refresh()
      })
    },
    updateStatus(id, status) {
      Api.updateIotDevice({ ids: [id], status }).then(() => {
