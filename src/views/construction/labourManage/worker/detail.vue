        ...
        <el-tab-pane name="attend" label="考勤记录" lazy>
          <el-card>
            ...
            <Grid
              class="check"
              :pageUseQuery="false"
              :request="checkingLogRequest"
              :row-class-name="({ row }) => (row.isHigh ? 'current' : '')"
            >
              <vxe-table-column type="seq" title="序号" width="60px" class-name=""></vxe-table-column>
...
<script>
import healthyApi from '../../healthy/api/work-bench'
...
export default {
  ...
  data() {
    return {
      ...
      healthLogRequest(body) {
        return healthyApi.getLabourMonitorList({
      ...
      checkingLogRequest(body) {
        return healthyApi.getLabourAttendList({
          projectId: localStorage.getItem('projectId'),
          employeeId: this.$route.params.id,
          ...body
        })
  ...
  created() {
    labourApi.getLabourEmployeeDetail(this.id).then(res => {
      this.form = res.data
    })
  },
  methods: {
