<template>
  <div style="background-color: unset; width: 1130px; margin: 0 auto">
    ...
    <div class="footer-divide">
      ...
      <el-button v-if="step === 2" size="medium" type="primary" :loading="btnSaveLoading" @click="submit">{{
        id ? '保存' : '创建'
      }}</el-button>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async submit() {
      ...
      if (this.id) {
        Api.updateLabourEmployee(this.form).then(res => {
          this.btnSaveLoading = false
          this.$message.success('编辑成功')
          this.$router.back()
        })
      } else {
        Api.addLabourEmployee(this.form).then(res => {
          this.btnSaveLoading = false
