  ...
  <div class="wrap">
    <el-card class="slide">
      ...
      <div class="tree-wrapper" style="padding: 10px; height: calc(100% - 100px)">
        <BimTree :modelReady="isReady" :is-model="false" :model-id="modelId" @changeBox="treeCheckChange" />
        <!--        <el-tree-->
        <!--          ref="tree"-->
        <!--          :data="treeData"-->
        <!--          node-key="id"-->
    ...
    <div class="content">
      <el-card>
        <div class="model-wrapper">
          <div class="model">
            <BimView ref="bim" :model-id="modelId" @ready="bimReady" />
          </div>
...
export default {
  ...
  methods: {
    ...
    bimReady() {
      this.allEleAry = window.allEleAry
      this.isReady = true
      this.getTreeData()
    },
    ...
    //树选中
    treeCheckChange(data, eleAry, isCheck) {
      let eleIdAry = eleAry.map(e => e.id)
      if (isCheck) {
        this.allEleAry = [...this.allEleAry, ...eleAry]
      } else {
        this.allEleAry = this.allEleAry.filter(a => !eleIdAry.includes(a.id))
      }
      this.qualityForm = {
        ...this.qualityForm,
        ...{
          bindDTOList: _.map(this.allEleAry, c => _.pick(c, ['docId', 'elementId']))
        }
      }
      this.safeForm = {
        ...this.safeForm,
    ...
    async getProgressModel() {
      ...
      if (cycleType === 3) {
        resData.map(r => {
          ...
          bimApp.setElementsColor(r.componentIdList, color)
        })
        return
      }
      resData.map(r => {
