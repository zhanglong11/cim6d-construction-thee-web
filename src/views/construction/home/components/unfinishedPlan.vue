  ...
  <el-card>
    ...
    <div class="unfinished-con">
      <div class="unfinished-th">
        ...
        <span class="receive-person">当前执行人</span>
        <span class="action">操作</span>
      </div>
      <ul class="unfinished-plan">
        <li v-for="(items, i) in unfinishedPlan" :key="i">
          ...
          >
            ...
            <p class="receive-person">{{ item.executePersonName }}</p>
            <el-button v-if="item.status === 0" type="primary" size="mini" @click="receive(item)">领取</el-button>
            <p v-else class="status">已领取</p>
          </div>
        </li>
