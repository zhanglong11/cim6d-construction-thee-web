    ...
    <div class="video-play">
      <el-card class="box-card">
        ...
        <div class="video-player big-video-img vjs-custom-skin">
          <Video v-if="activeUrl" :url="activeUrl" />
...
export default {
  ...
  methods: {
    ...
    async getVideoUrl(item) {
      const result = await Api.getVideoUrl({ factoryNum: item.factoryNum, deviceNum: item.deviceNum })
      let datas = result.data
      this.activeUrl = datas ? datas.WS_FLV : ''
    },
    // 播放
    async touch() {
      await Api.touch({ factoryNum: this.activeItem.factoryNum, deviceNum: this.activeItem.deviceNum })
    },
    // 清除定时器
    clearTouch() {
