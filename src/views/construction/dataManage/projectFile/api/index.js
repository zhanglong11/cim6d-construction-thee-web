...
export default {
  file: {
    list: () => http.file.get(`/document/category/list`),
    add: data => http.file.post(`/document/category/add`, data),
    update: data => http.file.post(`/document/category/update`, data),
    delete: id => http.file.get(`/document/category/delete/${id}`),
    imageAdd: data => http.file.post(`/image/category/add`, data),
    imageUpdate: data => http.file.post(`/image/category/update`, data)
  },
  doc: {
    // 获取文件目录树结构
