  ...
  <div class="no-footer">
    ...
    <template v-if="tabType === 'list'">
      ...
      >
        ...
        <vxe-table-column title="文件名" min-width="200" align="left">
          <template #default="{ row }">
            <span class="row-name">
              ...
              <span @dblclick="handleFolderClick(row)">{{ row.name }}</span>
            </span>
          </template>
          <template #header="{}">
        ...
        <vxe-table-column title="操作" field="opts" width="380" fixed="right">
          <template #default="{ row }">
            <el-button
              v-if="hasDownload"
              type="text"
            ...
            <el-button v-if="hasCopy" type="text" style="color: #c1a542" icon="el-icon-sort" @click="handleCopy(row)">
              复制
            </el-button>
            <el-button v-if="hasMove" type="text" style="color: #5a579e" icon="el-icon-rank" @click="handleMove(row)"
              >移动</el-button
            >
            <el-button
              v-if="hasRename"
    ...
    />
    <UploadDialog v-if="uploadVisible" :visible.sync="uploadVisible" :parentId="currentFolderId" :typeId="typeId" />
    <!--重命名-->
    <RenameDialog v-if="renameVisible" :visible.sync="renameVisible" :info="selectRow" @submit="handleRenameSubmit" />
    <!--新建文件夹-->
    <AddDialog v-if="addVisible" :visible.sync="addVisible" @submit="handleAddSubmit" />
    <PreviewDialog
      v-if="previewVisible"
...
<script>
import Api from '../../api'
import MultipleOpt from '../MultipleOpt'
import LevelTreeDialog from '../LevelTreeDialog'
...
export default {
  name: 'CommonDoc',
  components: { MultipleOpt, LevelTreeDialog, UploadDialog, RenameDialog, AddDialog, SelectDateRange, PreviewDialog },
  props: {
    typeId: {},
    hasAdd: {
      type: Boolean,
  ...
  methods: {
    ...
    async handleLevelTreeSubmit(id) {
      ...
      if (this.optType === 1) {
        await Api.doc.move({
          parentId: id,
          ids: this.isMultipleOpt ? this.selectRowIds : [this.selectRow.id],
          categoryId: this.typeId
        })
    ...
    async handleAddSubmit(data) {
      await Api.doc.add({ ...data, parentId: this.currentFolderId, categoryId: this.typeId })
