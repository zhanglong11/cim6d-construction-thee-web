  ...
  <div>
    ...
    >
      <template v-if="!isOnlyButton">
        <i class="el-icon-upload" :style="{ marginTop: fileType ? '20px' : '40px' }" />
    ...
    <p v-if="fileType" class="accept">
      支持扩展名: <span>{{ fileType }}</span>
    </p>
  </div>
</template>

<script>
import { projectFile, projectFileUploadUrl } from '@/utils/axios'
import axios from 'axios'
import { getToken } from '@/utils/auth'
import { getFileUrlByIds } from '../api'
export default {
  name: 'Upload',
  inject: {
    elForm: {
  ...
  methods: {
    ...
    changeUpload(file, fileList) {
      ...
      if (this.raw && this.fileType) {
        fileList = fileList.filter(e => {
          if (!_.some(this.fileType.split(','), string => e.name.toLowerCase().includes(string.toLowerCase()))) {
            this.$message.error('文件格式错误，只允许' + this.fileType)
            return false
          }
          return true
        })
    ...
    async handleUpload(options) {
      ...
      formData.append('categoryId', this.typeId)
      formData.append('parentId', this.folderId)
      const CancelToken = axios.CancelToken
      const self = this
      const result = await projectFile
    ...
    beforeUpload(file) {
      ...
      if (this.fileType) {
        if (!_.some(this.fileType.split(','), string => file.name.toLowerCase().includes(string.toLowerCase()))) {
          this.$message.error('文件格式错误，只允许' + this.fileType)
          return false
        }
      }
