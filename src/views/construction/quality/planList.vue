  ...
  <div class="container">
    ...
    <div class="tooltips">
      <el-form inline>
        ...
        <el-form-item>
          <el-button v-if="$hasPower('QualityPlanAdd')" type="primary" icon="el-icon-plus" @click="addItem()"
            >新建</el-button
          >
        </el-form-item>
      </el-form>
      ...
      <vxe-table-column title="操作" width="200" fixed="right">
        <template #default="{ row }">
          ...
          <template v-if="row.status === 2">
            <el-button
              v-if="$hasPower('QualityPlanStop') && row.inspectedNum === 0 && userId === row.creator"
              type="text"
              @click="stop(row.id)"
              >停用</el-button
            >
            <el-button
              v-if="
                $hasPower('QualityPlanInspect') &&
                row.inspectedNum !== row.inspectNum &&
                userId === row.inspectPrincipal
              "
              type="text"
          ...
          <template v-if="row.status === 1">
            <template v-if="row.inspectedNum === 0 && userId === row.creator">
              <el-button v-if="$hasPower('QualityPlanStart')" type="text" @click="start(row.id)">启用</el-button>
              <el-button v-if="$hasPower('QualityPlanEdit')" type="text" @click="addItem(row.id)">编辑</el-button>
              <el-button v-if="$hasPower('QualityPlanDelete')" type="text" @click="toDelete(row)">删除</el-button>
            </template>
          </template>
          <el-button v-if="$hasPower('QualityPlanDetail')" type="text" @click="toDetail(row, false)">查看</el-button>
        </template>
      </vxe-table-column>
    </Grid>
...
export default {
  ...
  methods: {
    ...
    async handleSubmit(isSave, data) {
      ...
      let params = { ...data, planType: 1, projectId: this.projectId }
