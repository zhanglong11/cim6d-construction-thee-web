    ...
    <div class="wrap">
      <el-card class="aside" header="已上传进度文件">
        <template #header>
          <div class="flex aic jcb">
            <span>已上传进度文件</span>
            <el-button v-if="$hasPower('ScheduleAdd')" type="primary" @click="$router.push('view/add')"
              >新增进度文件</el-button
            >
          </div>
        </template>
        <ul class="nav-list">
          ...
          >
            ...
            <div v-if="$hasPower('EpcPppScheduleRemove')" class="btn-group">
              <el-button type="danger" size="mini" @click="remove(e)"> 删除 </el-button>
            </div>
          </li>
        </ul>
      </el-card>
      <el-card style="flex: 1" header="进度文件" class="gantt-card">
        ...
        <el-button
          ...
          >保存</el-button
        >
        <el-button type="primary" @click="addInspectionDialogVisible = true">质量验收申请</el-button>
        <Gantt
          ref="gantt"
          :unicode="activeFile.unicode"
          style="height: 700px; margin-top: -35px"
          @refresh="getGanttData"
        ></Gantt>
      </el-card>
    </div>
    <AddInspection
      v-if="addInspectionDialogVisible"
      :visible.sync="addInspectionDialogVisible"
      @submit="addInspectionDialogVisible = false"
    ></AddInspection>
  </div>
</template>
...
import api from '../api'
import { ganttData2customData, customData2ganttData } from './utils'
import AddInspection from '../../quality/components/addInspection'

export default {
  name: 'ScheduleView',
  components: { Gantt, AddInspection },
  data: function () {
    return {
      //左侧列表
      navList: [],
      ...
      taskList: [],
      showGantt: true,
      addInspectionDialogVisible: false
    }
  },
  watch: {
    activeFile(val) {
