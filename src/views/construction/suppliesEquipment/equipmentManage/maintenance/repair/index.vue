  ...
  <div>
    <div class="tooltips">
      <el-form ref="pageQuery" :model="filterForm" inline>
        ...
        <el-form-item>
          <el-button-group v-if="$hasPower('EquipmentRepair')">
        ...
        },
        {
          ...
          slots: {
            default: ({ row }) => [
              ...
              row.repairStatus === 0 && this.$hasPower('EquipmentRepairDelete') ? (
                <el-button type="text" onClick={() => this.remove([row])}>
                  删除
                </el-button>
              ) : null,
              this.$hasPower('EquipmentRepairDetail') ? (
  ...
  methods: {
    ...
    toEdit(id) {
      _.isNil(id)
        ...
        : this.$router.push({ name: `EquipmentRepairEdit`, query: { id } })
    },
    toDetail(id) {
      this.$router.push({ name: `EquipmentRepairDetail`, query: { id, isCheck: false } })
    },
    toBasicDetail(row) {
      row.handleState
        ? this.$router.push({ name: `EquipmentRepairDetail`, query: { id: row.id } })
        : this.$router.push({ name: `EquipmentRepairBasicDetail`, query: { id: row.id } })
    },
