<template>
  <div class="container no-padding is-footer">
        ...
        <el-card v-if="item.modelType === 1" class="box-card">
          ...
          <el-form label-width="140px" class="form-con">
            ...
            <el-row>
              ...
              <el-col :md="12">
                <el-form-item label="外委单位">{{ item.outCompany || '无' }}</el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :md="12">
    ...
    <!--    执行维修-->
    <el-card
      v-if="(formData.handleState === 0 && isShow === 1) || (formData.handleState === 3 && isShow === 1)"
      class="box-card"
    >
      <div slot="header" class="clearfix details-header">
        <span class="details-title">处理情况</span>
        ...
        <el-col :span="10">
          <el-form ref="form" :model="form" :rules="rules" label-width="120px">
            ...
            <el-form-item label="是否停机" prop="isClose">
              <el-radio-group
                v-model="form.isClose"
                @change="getIsCloseVal"
                @input="val => (rules.downTime[0].required = val === 1)"
              >
            ...
            <el-form-item label="是否外委" prop="isOutsource">
              <el-radio-group
                ...
                @input="val => (rules.outCompany[0].required = val === 1)"
                @change="changeOutSourseVal"
              >
                <el-radio v-for="item in toggle" :key="item.value" :label="item.value">{{ item.label }}</el-radio>
              </el-radio-group>
            </el-form-item>
    ...
    <div class="footer-btn">
      ...
      <el-button
        ...
        >保存</el-button
      >
      <el-button v-if="isCheck && formData.repairStatus === 1 && isShow === 1" type="primary" @click="handleAudit"
        >审核</el-button
      >
      <el-button v-if="formData.handleState === 1 && isShow === 1" size="lg" type="primary" @click="accept"
        >验收</el-button
      >
    </div>
...
<script>
import Api from '../../../api/maintenance/repair'
...
export default {
  ...
  methods: {
    ...
    //改变外委的值
    changeOutSourseVal(val) {
      if (val === 2) {
        this.form.outCompany = ''
      }
    },
    toEdit() {
      this.$router.push({ name: `EquipmentRepairEdit`, query: { id: this.id } })
    },
    wrapperInfo() {
      ...
      this.detailInfo.approveContent.push({ tableKey: '设备维修单号', tableValue: info.code })
      this.detailInfo.approveContent.push({ tableKey: '设备名称', tableValue: info.equipmentName })
      this.detailInfo.approveContent.push({ tableKey: '设备编码', tableValue: info.equipmentCode })
      this.detailInfo.approveContent.push({ tableKey: '使用部门', tableValue: info.useDepartMent })
      this.detailInfo.approveContent.push({ tableKey: '规格型号', tableValue: info.models })
      this.detailInfo.approveContent.push({ tableKey: '设备类别', tableValue: this.$getLabel(this.level, info.level) })
    },
    //审核流程附件
    setAccessoryInfo() {
      let flag = true
