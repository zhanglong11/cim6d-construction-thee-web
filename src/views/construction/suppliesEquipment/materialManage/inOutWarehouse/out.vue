<template>
  <div>
    <el-form ref="form" :model="formData" inline>
      ...
      <el-form-item v-if="$hasPower('OutWarehouse')">
        <el-button-group>
          <el-button type="primary" icon="el-icon-search" @click="onSearch()">搜索</el-button>
          <el-button icon="el-icon-refresh-left" @click="onReset">重置</el-button>
        </el-button-group>
      </el-form-item>
      <el-form-item v-if="$hasPower('AddOutWarehouse')">
        <el-button type="primary" icon="el-icon-plus" @click="onNew">添加出库</el-button>
      </el-form-item>
    </el-form>
    ...
    >
      ...
      <el-table-column label="操作" min-width="100">
        <template slot-scope="scope">
          <el-button v-if="$hasPower('OutWarehouseDetail')" type="text" @click="toDetail(scope.row.id)">查看</el-button>
          <el-button v-if="$hasPower('OutstoreOrderEdit')" type="text" @click="toEdit(scope.row.id)">编辑</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  methods: {
    ...
    toDetail(id) {
      this.$router.push({
        path: `/supplies/materialManage/outWarehouseDetail`,
        query: {
          id: id
        }
      })
    ...
    toEdit(id) {
      this.$router.push({
        path: `/supplies/materialManage/editOutWarehouse`,
        query: {
          id: id
        }
      })
    ...
    onNew() {
      this.$router.push({ path: `/supplies/materialManage/addOutWarehouse` })
    }
  }
}
</script>
