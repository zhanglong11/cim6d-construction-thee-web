<template>
  <div class="container no-padding is-footer">
    ...
    <div class="footer-btn">
      ...
      <el-button size="lg" type="primary" :loading="btnSaveLoading" @click="submit(0)">保存</el-button>
      <el-button size="lg" type="primary" :loading="btnSaveSubmitLoading" @click="submit(1)">保存并提交</el-button>
    </div>
    <!--选择工序-->
...
export default {
  ...
  methods: {
    ...
    rowClassName({ row, rowIndex }) {
      if (row.inventoryNumber < row.number) {
        return 'col-red'
      }
    },
    //选择工序弹窗
    ...
    getTarget(e) {
      ...
      let tableData = [...this.tableData, ...targetList]
      this.tableData = _.unionBy(
        _.filter(tableData, item => item.id),
        'id'
      )
    },
    //选择工序回传
...
.vxe-table--body .vxe-body--row.col-red {
  background-color: rgba(252, 10, 15, 0.7);
  color: #fff;
}
</style>
