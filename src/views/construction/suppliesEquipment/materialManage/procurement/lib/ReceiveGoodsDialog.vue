...
export default {
  ...
  methods: {
    async submit() {
      ...
      await this.$refs.xTable111.validate(true, valid => {
        if (valid) {
          Object.values(valid).forEach(errList => {
          ...
          this.$emit('getTarget', { checked: this.tableData, nextPerson: this.form })
