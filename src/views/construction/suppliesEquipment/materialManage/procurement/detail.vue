<template>
  <div class="container no-padding is-footer">
      ...
      <div class="details">
        <el-row>
          ...
          <el-col :md="24">
            ...
            <span>{{ info.appyArrivalTime | ymd }}</span>
          </el-col>
          <el-col :md="24">
    ...
    <div class="footer-btn">
      ...
      <el-button
        v-if="info.status === 1 && info.currentPersonId === $store.state.user.id"
        size="lg"
        type="primary"
      ...
      >
      <el-button
        v-if="info.status === 2 && info.currentPersonId === $store.state.user.id"
        size="lg"
        type="primary"
      ...
      >
      <el-button
        v-if="info.status === 4 && info.currentPersonId === $store.state.user.id"
        size="lg"
        type="primary"
        @click="receiveGoodsVisible = true"
        >到货通知</el-button
      >
      <el-button
        v-if="info.status === 5 && info.currentPersonId === $store.state.user.id"
        size="lg"
        type="primary"
        @click="handleStorage"
        >执行入库</el-button
      ...
      <el-dialog title="审核" :visible.sync="checkVisible" width="500px" :modal-append-to-body="false">
        <el-form ref="form" :model="form" label-width="120px" :rules="rules">
          ...
          <!--精装修 独有 start-->
          <el-form-item v-if="form.auditStatus === '1'" label="是否继续审核">
            <el-radio-group v-model="form.isContinueAudit">
          ...
          <el-form-item
            ...
            :label="form.isContinueAudit === '0' ? '采购人' : '下一审核人'"
            prop="nextPersonId"
          >
            <SelectUser
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" :loading="btnAuditLoading" @click="handleCheck">确 定</el-button>
        </div>
      </el-dialog>
      <!--审核弹窗 end-->
      <!--精装修独有 采购 start-->
  ...
  methods: {
    ...
    toEdit(id) {
      ...
      _.isNil(id)
        ...
        : this.$router.push({ name: `ProcurementEdit`, params: { id } })
    },
    // 审核中 进行审核
    async handleCheck() {
