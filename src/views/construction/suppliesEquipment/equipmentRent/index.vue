  ...
  <div>
    <div class="tooltips">
      <el-form ref="pageQuery" :model="filterForm" inline>
        ...
        <el-form-item>
          <el-button-group v-if="$hasPower('EquipmentRent')">
        ...
        <el-form-item>
          <el-button
            v-if="$hasPower('EquipmentRentAdd')"
            type="primary"
            icon="el-icon-plus"
    ...
    <Grid ref="table" :request="request" :filter-form.sync="filterForm">
      ...
      ></vxe-table-column>
      <vxe-table-column title="操作" width="200px" fixed="right">
        <template #default="{ row }">
          <router-link v-if="$hasPower('EquipmentRentDetail')" type="text" :to="'detail?id=' + row.id"
            >查看</router-link
          >
          <router-link v-if="row.status === 0 && $hasPower('EquipmentRentEdit')" type="text" :to="'edit?id=' + row.id"
            >编辑</router-link
          >
          <el-button v-if="row.status === 0 && $hasPower('EquipmentRentDelete')" type="text" @click="remove(row)"
            >删除</el-button
          >
        </template>
      </vxe-table-column>
