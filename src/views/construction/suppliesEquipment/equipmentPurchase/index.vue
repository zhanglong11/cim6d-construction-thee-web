  ...
  <div>
    <div class="tooltips">
      <el-form inline>
        ...
        <el-form-item>
          <el-button-group v-if="$hasPower('PurchaseList')">
...
export default {
  ...
  methods: {
    ...
    handleDetail(id) {
      this.$router.push({ name: `EquipmentPurchaseDetail`, query: { id, isCheck: false } })
    },
    handleEdit(id) {
      this.$router.push({ name: `EquipmentPurchaseEdit`, query: { id } })
    },
    async handleDelete(id) {
      await this.$confirm('此操作将删除该条数据, 是否继续?', '提示', {
        ...
        .then(() => {
          Api.deleteEquipmentPurchase(id).then(res => {
