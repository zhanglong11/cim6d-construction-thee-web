...
var ACTIVITI_SERVICE_TASK_LIKE = ['activiti:class', 'activiti:delegateExpression', 'activiti:expression']
...
function createActivitiFieldInjections(template, bpmnFactory) {
  ...
  template.properties.forEach(function (p) {
    ...
    if (bindingType === 'activiti:field') {
      injections.push(createActivitiFieldInjection(binding, p.value, bpmnFactory))
    }
  })
...
function createActivitiProperties(template, bpmnFactory) {
  ...
  template.properties.forEach(function (p) {
    ...
    if (bindingType === 'activiti:property') {
      properties.push(createActivitiProperty(binding, p.value, bpmnFactory))
    }
  })
...
function createActivitiInOut(template, bpmnFactory) {
  ...
  template.properties.forEach(function (p) {
    ...
    if (bindingType === 'activiti:in') {
      inOuts.push(createActivitiIn(binding, p.value, bpmnFactory))
    } else if (bindingType === 'activiti:out') {
      inOuts.push(createActivitiOut(binding, p.value, bpmnFactory))
    } else if (bindingType === 'activiti:in:businessKey') {
      inOuts.push(createActivitiInWithBusinessKey(binding, p.value, bpmnFactory))
    }
  })
...
function createActivitiExecutionListeners(template, bpmnFactory) {
  ...
  template.properties.forEach(function (p) {
    ...
    if (bindingType === 'activiti:executionListener') {
      executionListener.push(createActivitiExecutionListenerScript(binding, p.value, bpmnFactory))
    }
  })
