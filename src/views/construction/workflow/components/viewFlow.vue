<template>
  <div v-if="!hiddenFlow">
    <el-card class="box-card">
      <el-button-group v-if="isExport" style="float: right; position: relative; z-index: 1">
...
export default {
      ...
      fileRecover: [
        {
          ...
          fileName: 'test.jpg',
          fileNameSuffix: 'jpg',
          fileUrl: `/cim6d-file${process.env.VUE_APP_BASE_API}/noToken/file/commonFile/download/2cf75a22afd94abeba4080357ebbf3c7`
        }
      ],
  ...
  methods: {
            ...
            if (!item.options.hidden && (item.type === 'fileupload' || item.type === 'imgupload')) {
              ...
              defaultValueList.map(e => {
                ...
                newList.push({
                  ...
                  fileUrl: `/cim6d-file${process.env.VUE_APP_BASE_API}/noToken/file/commonFile/download/${fileKey}`
                })
              })
    ...
    // 导出
    handleExport() {
      this.$emit('setAccessoryInfo')
      // console.log(this.info)
      this.axios.file
        .post(`/jasper/report/commonCommonApprove/create`, this.info, { responseType: 'blob' })
        .then(res => {
          saveAs(res.data, this.info.approveName + '.pdf')
        })
    },
    // 预览
    handlePreview() {
      ...
      this.axios.file
        ...
        .then(res => {
          ...
          this.$nextTick(() => {
            ...
            this.$refs.iframe.src = window.URL.createObjectURL(blob)
          })
        })
    },
    // 取消预览
