<template>
  <el-dialog
    top="10vh"
    :title="`添加角色（${selectUserIds.length}/${max}）`"
    width="50%"
    append-to-body
    :visible.sync="modalStatus"
  >
    <el-form ref="formRef">
      <el-form-item v-if="selectUserIds.length > 0" label="已选角色">
        <div class="tags">
  ...
  computed: {
    list() {
      ...
      treeForEach(data, e => {
        e.label = e.name
        e.id = e.id
        e.isUser = e.organType === 5
        e.disabled = e.organType !== 5 // 禁止部门选中事件
        if (e.isUser) {
          delete e.children
        }
      })
  ...
  methods: {
    ...
    changeTree(e, { checkedNodes, checkedKeys, halfCheckedNodes, halfCheckedKeys }) {
      ...
      checkedNodes.map(item => {
        if (item.organType === 5 && !selectUserIds.includes(item.id)) {
      ...
      if (selectUserIds.length > this.max) {
        this.$message.error(`最多选择 ${this.max} 个角色！`)
        this.$nextTick(() => {
    ...
    deleteUser(item, index) {
      ...
      this.$nextTick(() => {
        this.$refs.treeRef.setCheckedKeys(this.selectUserIds)
      })
    },
    // 确定
    saveModal() {
