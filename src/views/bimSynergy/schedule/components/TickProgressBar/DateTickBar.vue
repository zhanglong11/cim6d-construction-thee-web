...
export default {
  ...
  methods: {
    ...
    initBar() {
      const canvas = this.$refs.canvas
      // this.width = canvas.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.getBoundingClientRect().width - 20
      let node = canvas.parentElement
      while (node !== null) {
        if (node.className === 'middle') {
          // 父级容器的className为middle时，取该父级容器的宽度
          this.width = node.getBoundingClientRect().width - 20
          node = null
        } else {
