          ...
          <el-form-item label="" prop="constructionSituation" class="model-list">
            ...
            <ul>
              <li
                ...
                :key="item.id"
                :class="{ 'add-class': item.isCompanyActive && isCompanyActive === index }"
...
export default {
  ...
  methods: {
    ...
    handleCompanyModelList(list, index) {
      if (!list.isCompanyActive) {
        list.isCompanyActive = true
        this.isCompanyActive = index
    ...
    async handleSave() {
      this.handleCancel()
      let isCopy = this.checkedList.id ? true : false
      this.$router.push({
    ...
    // 获取详情
    getModelFormList() {
      Api.getFormList({ scope: 1, status: 1, projectType: this.projectType }).then(res => {
        ...
        this.list.unshift({
          icon: require('../../../assets/images/projectOA/icon/add-model.png'),
          name: '不使用模板',
          isActive: true,
          formType: 1
        })
      ...
      Api.getFormList({ scope: 0, status: 1, projectType: this.projectType }).then(res => {
        console.log(33333, res.data.records)
        this.companyModelList = res.data.records || []
        this.companyModelList.unshift({
          icon: require('../../../assets/images/projectOA/icon/add-model.png'),
          name: '不使用模板',
          formType: 1
        })
        ...
        // console.log('list', this.companyModelList)
      })
    }
  }
}
