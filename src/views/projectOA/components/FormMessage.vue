  ...
  <div class="container">
    <el-card v-for="(item, index) in menuList" :key="index" class="box-card">
      <div class="config-list">
        ...
        <div v-if="item.menuConfig === 2">
          <el-button
            v-if="$hasPower('SetFormManageFlag')"
            type="primary"
            icon="el-icon-setting"
            @click="configCustomForm(item.menuConfig)"
            >管理固定表单</el-button
      ...
      <template v-if="item.menuConfig === 0">
        <ul>
          <li
            v-for="subItem in item.list"
            :key="subItem.id"
            :class="{ 'is-system': subItem.flag === 2 }"
            @click="toDetails(subItem, true)"
          >
            <!-- <div v-if="subItem.flag === 1">
              <span class="circle" :class="{ blue: subItem.projectId == null }"></span>
              <span class="sign">{{ subItem.projectId == null ? '企业' : '项目' }}</span>
            </div> -->

            <div v-if="subItem.formType === 1">
            ...
            <div class="list">
              ...
              <div class="right-word">
                <p class="title">
                  <span class="title-name">{{ subItem.name }}</span>
                </p>
                <el-tooltip v-if="subItem.description" :content="subItem.description" placement="bottom">
    ...
    <!--    管理自定义表单 start-->
    <el-dialog
      :visible.sync="configCustomFormVisible"
      :title="configCustomFlag === 1 ? '管理自定义表单' : '管理固定表单'"
      width="90%"
    >
      <div class="custom-roll">
        <el-button v-if="configCustomFlag === 1" type="primary" icon="el-icon-plus" @click="handleGroupAdd(null)"
          >新建小组</el-button
        >
        <div
          ...
          <h4 class="form-title">
            ...
            ><span v-if="subItem.flag === 1" style="display: inline-block; margin: 0 0 10px 10px">
              <el-button
                ...
                >删除小组</el-button
              >
              <el-button type="primary" icon="el-icon-edit" @click="handleGroupAdd(subItem)">重命名</el-button>
            </span>
          </h4>
          <draggable
...
import Api from '@/views/projectOA/api'
export default {
  name: 'FormMessage',
  components: {
    addCustomForm,
  ...
  methods: {
    ...
    messageFormSubmit() {
      ...
      let target = params.map((item, index) => {
        return {
          ...item,
          sort: index
        }
    ...
    configCustomForm(flag) {
      ...
      if (flag === 1) {
        ...
        let configFixedFormList = _.cloneDeep(this.configFixedFormList)
    ...
    //获取最近使用的form表单
    getRecentlyFormList() {
      Api.getRecentlyFormList({}).then(res => {
        let list = res.data || []
        this.recentlyFormList = _.uniqBy(list, i => i.name)
        this.recentlyFormList = this.recentlyFormList.filter(e => e.name)
        /*this.recentlyFormList = _.toPairs(_.groupBy(list, 'groupName')).map(function (item) {
          return {
            name: item[0],
            subList: item[1]
          }
    ...
    getModelFormList() {
      Api.getGroupFormList({ flag: '' }).then(res => {
        let list = res.data || []
        let customForm = list.filter(item => item.flag === 1)
        let fixForm = list.filter(item => item.flag === 2)
        fixForm.map(e => {
          if (e.children) {
            e.children = _.filter(e.children, o => {
              return this.$hasPower(o.powerKey)
            })
          }
        })
        this.configCustomFormList = customForm
        fixForm = _.filter(fixForm, o => {
          return o.children && o.children.length > 0
        })
        this.configFixedFormList = fixForm
        this.menuList = [
          {
            menuConfig: 0,
            name: '最近使用',
            list: this.recentlyFormList
          },
          {
            menuConfig: 1,
            name: '自定义表单',
