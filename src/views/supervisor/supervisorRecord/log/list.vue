  ...
  <div>
    ...
    <div class="tooltips">
      <el-form ref="form" :model="filterForm" inline>
        ...
        <el-form-item>
          <el-button v-if="$hasPower('SupervisionRecordLogAdd')" type="primary" icon="el-icon-plus" @click="handleAdd()"
            >新建监理日志</el-button
          >
        </el-form-item>
      </el-form>
    ...
    <Grid ref="table" :request="request" :filter-form.sync="filterForm">
      ...
      <vxe-table-column field="authorDate" title="星期" min-width="80">
        <template slot-scope="{ row }">
          <span>{{ getWeekDay(row.reportDate) }}</span>
        </template>
      </vxe-table-column>
      <vxe-table-column field="weather" title="日气象" min-width="80" />
      <vxe-table-column field="maximumTemperature" title="最高温度" min-width="80" />
      <vxe-table-column field="minimumTemperature" title="最低温度" min-width="80" />
      <vxe-table-column field="creatorName" title="日志填写人" min-width="80" />
      ...
      <vxe-table-column title="操作" width="180" fixed="right">
        <template slot-scope="scope">
          <template v-if="scope.row.status === 0">
            <el-button
              v-if="$hasPower('SupervisionRecordLogEdit')"
              type="text"
              size="mini"
            ...
            >
            <el-button
              v-if="$hasPower('SupervisionRecordLogDelete')"
              type="text"
              size="mini"
            ...
            >
            <el-button
              v-if="$hasPower('SupervisionRecordLogSign') && scope.row.signPersonId === $store.state.user.id"
              type="text"
              size="mini"
          ...
          <el-button
            v-if="$hasPower('SupervisionRecordLogDetail')"
            type="text"
            size="mini"
