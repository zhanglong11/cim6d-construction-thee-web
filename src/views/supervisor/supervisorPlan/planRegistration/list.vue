  ...
  <div>
    ...
    <div class="tooltips">
      <el-form ref="form" :model="queryForm" inline>
        ...
        <el-form-item>
          <el-button v-if="$hasPower('PlanRegistrationAdd')" type="primary" icon="el-icon-plus" @click="handleAdd()"
            >新建</el-button
          >
        </el-form-item>
      </el-form>
    ...
    <el-table v-loading="loading" :data="tableData" border stripe size="small">
      ...
      <el-table-column prop="name" label="监理规划名称" show-overflow-tooltip />
      ...
      <el-table-column label="操作" width="200" fixed="right" align="center">
        <template slot-scope="scope">
          <el-button
            v-if="scope.row.status === 0 && $hasPower('PlanRegistrationEdit')"
            type="text"
            size="mini"
          ...
          >
          <el-button
            v-if="scope.row.status === 0 && $hasPower('PlanRegistrationDelete')"
            type="text"
            size="mini"
          ...
          >
          <el-button
            v-if="$hasPower('PlanRegistrationDetail')"
            type="text"
            size="mini"
    ...
    <!-- 添加监理规划模态框 start -->
    <AddPlanRegistrationDialog
      v-if="addPlanRegistrationVisible"
      :id="id"
      :visible.sync="addPlanRegistrationVisible"
      @refresh="
        () => {
          refresh()
        }
    ...
    <!-- 监理规划详情 start -->
    <DetailPlanRegistrationDialog
      v-if="detailPlanRegistrationVisible"
      :id="id"
      :visible.sync="detailPlanRegistrationVisible"
    />
    <!-- 监理规划详情 end -->
  </div>
</template>
...
import AddPlanRegistrationDialog from './addPlanRegistrationDialog'
import DetailPlanRegistrationDialog from './detailPlanRegistrationDialog'
export default {
  // import引入的组件需要注入到对象中才能使用
  name: 'PlanRegistrationList',
  components: { AddPlanRegistrationDialog, DetailPlanRegistrationDialog },
  data() {
    // 这里存放数据
    return {
      auditStatus,
      ...
      tableData: [],
      total: 0,
      addPlanRegistrationVisible: false,
      detailPlanRegistrationVisible: false,
      id: ''
    }
  },
  // 生命周期 - 挂载完成（可以访问DOM元素）
  ...
  methods: {
    ...
    // 新建
    handleAdd() {
      this.addPlanRegistrationVisible = true
      this.id = ''
    },
    // 编辑
    handleEdit(id) {
    ...
    handleDelete(id) {
      this.$confirm('确认删除该数据?', '提示', {
        ...
        .then(() => {
          Api.deletePlanRegistration(id).then(() => {
    ...
    // 查看
    handleDetail(id) {
      this.detailPlanRegistrationVisible = true
      this.id = id + ''
    },
    // 当前页码改变
