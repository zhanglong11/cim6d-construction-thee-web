<template>
  <el-dialog :title="id ? '修改' : '新建' + '监理规划'" :visible="visible" width="820px" @close="handleCancel">
    <el-form ref="form" :model="form" :rules="formRules" label-width="140px">
      <el-row>
        <el-col :md="12">
          <el-form-item label="监理规划名称" prop="name">
      ...
      <el-row>
        <el-col :md="12">
          <el-form-item label="监理规划备注" prop="remark">
      ...
      <el-row>
        <el-col :md="12">
          <el-form-item label="监理规划附件" prop="fileIds">
...
export default {
  ...
  name: 'AddPlanRegistrationDialog',
  props: {
    visible: Boolean,
    id: String
  },
    ...
    handleSave() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          Api.savePlanRegistration(this.form)
            .then(res => {
    ...
    handleSaveSubmit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          Api.saveAndSubmitRegistration(this.form)
            .then(res => {
              this.$message.success('操作成功')
              this.$emit('update:visible', false)
              this.$emit('refresh')
